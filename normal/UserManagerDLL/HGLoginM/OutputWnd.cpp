...
void COutputWnd::FillBuildWindow( CString outText )
{

	
	CTime time = CTime::GetCurrentTime();   ///构造CTime对象
	CString m_strTime = time.Format("%Y-%m-%d %H:%M:%S");
	outText.Format( _T("[ %s ] %s ") ,m_strTime,outText );
    m_wndOutputBuild.AddString( outText );
	m_wndOutputBuild.SendMessage( WM_KEYDOWN,VK_END,0 );
...
void COutputWnd::FillDebugWindow( CString outText )
{
	m_wndOutputDebug.AddString( outText );

	m_wndOutputDebug.SendMessage( WM_KEYDOWN,VK_END,0 );
}
...
void COutputWnd::FillFindWindow( CString outText )
{
	m_wndOutputFind.AddString( outText );
}
