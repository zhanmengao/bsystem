...
struct MonitorInfo
{
	...
	stdstr csSerialNum;
	int iSize;					//大小
	int Maximum_Vertical_Size;	
	int Maximum_Horizontal_Size;
	int index;
public:
	std::wstring Guid()
	{
		return WinStrUtils::GUID2Str(ClassGuid);
	}
	stdstr MonitorSize()
	{
		TCHAR buf[1024] = { 0 };
		wsprintf(buf, _T("%d英寸(%d厘米×%d厘米)"), iSize, Maximum_Horizontal_Size, Maximum_Vertical_Size);
		return buf;
	}
...
public:
	...
	BOOL GetMonitors(vector<MonitorInfo> &vecMyList);
...
protected: // Methods
	...
	// Gedif specific
	void		Bricolage(MonitorInfo &Monitor, Standard_EDID *myRecord);
