...
#if _DEBUG
#define SUM_FILE "sum.ini"
#define HW_FILE "hw.ini"
#else
#define SUM_FILE "sum.ini"
#define HW_FILE "hw.ini"
#endif
void CollectRoutinue::Awake()
{
	...
	mINISUM.SetPath(PathHelper::GetModuleFolder() + SUM_FILE);
	mINIHW.SetPath(PathHelper::GetModuleFolder() + HW_FILE);
	//电脑开机时间
...
{
	LOG_DAILY("CollectRoutinue Start Begin");
	netInfo = SysInfoUtils::GetNetInfo();								//缓存一份，后续不再取
	if (netInfo.strIp.empty() || netInfo.strMac.empty())
	{
		return false;
	}
	...
	TickEnd();
	LOG_DAILY("CollectRoutinue Start Sec");
	return true;
}
...
void CollectRoutinue::OnCommandCollStart(const std::string& orgCode)
{
	if (StartRoot.empty())
	{
		...
		{
			stdstr SystemProductName = mINISUM.ReadString("系统概述", "主板|主板名称", "*");
			stdstr serial_num = mINISUM.ReadString("系统概述", "DMI|DMI 系统序列号", "*");
			stdstr BIOSVersion = mINISUM.ReadString("系统概述", "DMI|DMI BIOS 版本", "*");
			StartRoot["wbRegisteredOwner"] = CONVERT(mINISUM.ReadString("系统概述", "DMI|DMI 系统制造商", "*"));
			StartRoot["MachineName"] = CONVERT(mINISUM.ReadString("系统概述", "计算机|计算机名称", "*"));
			...
			//系统序列号		主板名称  bios版本
			if (IsInvalidAIDAValue(serial_num) || IsInvalidAIDAValue(SystemProductName) || IsInvalidAIDAValue(BIOSVersion))
			{
				vector<WMIBIOS> biosvec;
				std::string exce;
				if (mWMI.Begin() && mWMI.GetBIOS(biosvec, exce))
				{
...
Json::Value CollectRoutinue::SendMonitInfo()
{
	...
	{
		char buffer[1024] = { 0 };
		sprintf_s(buffer, _countof(buffer), "[Catch exception]:%s : %s \n", __FUNCTION__, e.what());
		LOG(buffer);
		return instructRoot;
	}
}
...
void CollectRoutinue::TickEnd()
{
	...
	{
		char buffer[1024] = { 0 };
		sprintf_s(buffer, sizeof(buffer), "[Catch exception]:%s : %s \n", __FUNCTION__, e.what());
		LOG(buffer);
	}
}
	...
	try
	{
		...
		if (!msg->Message.empty())
		{
			auto func = [=](void* argv)->void* 
			{
				...
				if (msgBox)
				{
					//msgBox->Message = "Vesystem said :" + msgBox->Message;
