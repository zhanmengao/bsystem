...

{
         ...
         */
#if !defined(_KERNELX) && defined(_CRT_APP)
        if ( !GetFileInformationByHandleEx((HANDLE)_osfhnd(fildes), FileBasicInfo, &bfi, sizeof(bfi)) ) {
#else  /* !defined(_KERNELX) && defined(_CRT_APP) */
...
#if !defined(_KERNELX) && defined(_CRT_APP)
        if ( bfi.FileAttributes & FILE_ATTRIBUTE_READONLY )
#else  /* !defined(_KERNELX) && defined(_CRT_APP) */
         ...
         */
#if !defined(_KERNELX) && defined(_CRT_APP)
        if ( bfi.LastWriteTime.LowPart ||
             bfi.LastWriteTime.HighPart )
#else  /* !defined(_KERNELX) && defined(_CRT_APP) */
...
#if !defined(_KERNELX) && defined(_CRT_APP)
            if ( !FileTimeToSystemTime( (FILETIME*)&bfi.LastWriteTime, &SystemTime )
...
#if !defined(_KERNELX) && defined(_CRT_APP)
        if ( bfi.LastAccessTime.LowPart ||
             bfi.LastAccessTime.HighPart )
#else  /* !defined(_KERNELX) && defined(_CRT_APP) */
...
#if !defined(_KERNELX) && defined(_CRT_APP)
            if ( !FileTimeToSystemTime( (FILETIME*)&bfi.LastAccessTime, &SystemTime ) ||
                 !SystemTimeToTzSpecificLocalTime( NULL, &SystemTime, &SystemTime ) )
#else  /* !defined(_KERNELX) && defined(_CRT_APP) */
...
#if !defined(_KERNELX) && defined(_CRT_APP)
        if ( bfi.CreationTime.LowPart ||
             bfi.CreationTime.HighPart )
#else  /* !defined(_KERNELX) && defined(_CRT_APP) */
...
#if !defined(_KERNELX) && defined(_CRT_APP)
            if ( !FileTimeToSystemTime( (FILETIME*)&bfi.CreationTime, &SystemTime ) ||
                 !SystemTimeToTzSpecificLocalTime( NULL, &SystemTime, &SystemTime ) )
#else  /* !defined(_KERNELX) && defined(_CRT_APP) */
...
#if !defined(_KERNELX) && defined(_CRT_APP)
        /* use the file handle to get statandard info about the file, including file size
         */
        if ( !GetFileInformationByHandleEx((HANDLE)_osfhnd(fildes), FileStandardInfo, &sfi, sizeof(sfi)) ) {
            _dosmaperr(GetLastError());
            retval = -1;
            goto done;
        }
...
#if _USE_INT64
        buf->st_size = ((__int64)(sfi.EndOfFile.HighPart)) * (0x100000000i64) +
                       (__int64)(sfi.EndOfFile.LowPart);
#else  /* _USE_INT64 */
        buf->st_size = sfi.EndOfFile.LowPart;
#endif  /* _USE_INT64 */
#else  /* !defined(_KERNELX) && defined(_CRT_APP) */
#if _USE_INT64
