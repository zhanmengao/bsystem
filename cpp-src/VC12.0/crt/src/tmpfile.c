 ...
 */
#ifdef _UNICODE
#define _tinit_namebuf winit_namebuf
#else  /* _UNICODE */
#define _tinit_namebuf init_namebuf
#endif  /* _UNICODE */
static int __cdecl _tinit_namebuf(int);
...
#if defined(_KERNELX) && !defined(_UNICODE)
/***
*DWORD __cdecl __crtGetTempPathKernelXA(_In_ unsigned int bufferLength, _Out_writes_to_opt_(bufferLength, return + 1) char* buffer) - retrieves the temporary path
*
*Purpose:
*       Returns a string containing a system provided path for temporary files.
*
*Entry:
*       unsigned int bufferLength - the size of the output buffer, in characters
*       char* buffer - a user allocated buffer where the path is to be copied to
*
*Exit:
*       returns the size of the path in characters (not counting the null termination).
*       returns 0 on failure.
*
*Exceptions:
*
*******************************************************************************/
static DWORD __cdecl __crtGetTempPathKernelXA(_In_ unsigned int bufferLength, _Out_writes_to_opt_(bufferLength, return + 1) char* buffer)
{
    DWORD length = 0;
    char* temp = NULL;
    wchar_t wpath[MAX_PATH] = {0};

    if (!(length = GetTempPathW(MAX_PATH, wpath)))
    {
        return 0;
    }

    if (!__copy_to_char(wpath, &temp))
    {
        return 0;
    }

    if (memcpy_s(buffer, bufferLength, temp, length + 1) != 0)
    {
        _free_crt(temp);
        return 0;
    }
...
#endif /* defined(_KERNELX) && !defined(_UNICODE) */
...
static int __cdecl _tinit_namebuf(
        int flag
        )
{
        _TSCHAR *p = NULL, *q;
        _TSCHAR tempPathBuffer[MAX_PATH] = { 0 };
        DWORD retVal = 0;
        size_t size = 0;
        ...
        if (flag == _TMPFILE_BUFFER)
        {
            ...
            /* KernelX doesn't support GetTempPathA(). */
            retVal = __crtGetTempPathKernelXA(MAX_PATH, tempPathBuffer);
#endif  /* defined(_UNICODE) */
#elif defined(_CRT_APP)
#if defined(_UNICODE)
            retVal = __crtGetTempPathWinRTW(MAX_PATH, tempPathBuffer);
#else  /* defined(_UNICODE) */
            retVal = __crtGetTempPathWinRTA(MAX_PATH, tempPathBuffer);
#endif  /* defined(_UNICODE) */
