...
/*
 * number of entries in the string tables
 */
const int __rg_language_count = _countof(__rg_language);
const int __rg_country_count = _countof(__rg_country);
...
BOOL __cdecl __get_qualified_locale(const LPLC_STRINGS lpInStr, UINT* lpOutCodePage, LPLC_STRINGS lpOutStr)
{
    ...
    if (*_psetloc_data->pchCountry)
        TranslateName(__rg_country, __rg_country_count - 1,
    ...
    if (*_psetloc_data->pchLanguage)
    {
        ...
        if (_psetloc_data->iLocState == 0) {
            ...
            if (TranslateName(__rg_language, __rg_language_count - 1,
