...
volatile __declspec(appdomain) __MPNH __mpnhHeap= (__MPNH) EncodePointer(NULL);
volatile __declspec(appdomain) _new_handler_m __nhmHeap= (_new_handler_m) EncodePointer(NULL);
...
static void __clrcall _callnewh_cleanup(void)
{
    ...
    __MPNH enull = (__MPNH) EncodePointer(NULL);
...
{
    _PNH pnh=_query_new_handler();
    _new_handler_m enull = (_new_handler_m) EncodePointer(NULL);
...
public:
        __signal_init()
        {
                _PHNDLR_m enull = (_PHNDLR_m) EncodePointer(NULL);
