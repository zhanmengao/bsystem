...
extern "C" int __cdecl _ismbcblank_l(
        unsigned int c,
        _locale_t plocinfo
        )
{
        ...
        if (c > 0x00FF)
        {
            ...
            /* ensure single MB character and test for type */
            return (ctype[1] == 0 && ctype[0] & _BLANK);


        } else
        {
            return _ismbbblank_l( c, _loc_update.GetLocaleT());
        }
}

extern "C" int (__cdecl _ismbcblank)(
        unsigned int c
        )
{
        return _ismbcblank_l(c, NULL);
}
