...
// Introduce a banned stacktrace API for Debug CRT_APP
...
{
    // Don't do anything when targetting MSDK or _KERNELX (We could set it to active processor mask in the future)
#if !(defined(_CRT_APP) || defined(_KERNELX))
#if (defined(_M_IX86) || defined(_M_X64))
...
{
// TODO - KERNELX - When _CRT_APP is no longer defined for _KERNELX apps, the second clause should be removed
#if defined(_CRT_APP) && !defined(_KERNELX)
...
// TODO - KERNELX - When _CRT_APP is no longer defined for _KERNELX apps, the second clause should be removed
#if defined(_CRT_APP) && !defined(_KERNELX)
    // TODO: We only call Sleep(0) and Sleep(1)
    // In both the cases the intent is to yield
    // the quantum to some other thread...
    ASSERT(ms >= 0 && ms <= 1);
#else
    Sleep(ms);
#endif
...
{
#if defined(_KERNELX)
    // The pre-Vista timer APIs are not supported on _KERNELX
    ENSURE_NOT_APP_OR_KERNELX();
#elif defined(_CRT_APP)

    ASSERT(timerQueue == NULL);

    unsigned int delay = (period == 0) ? dueTime : period;
    bool periodic = (period == 0) ? false : true;
    auto newTimer = __crtCreateWinRTTimer(lpStartAddress, lpParameter, delay, periodic);
...
void __DeleteTimerQueueTimer(HANDLE timerQueue, HANDLE hTimer, HANDLE completionEvent)
{
    ...
    __crtDeleteWinRTTimer(hTimer, (completionEvent == INVALID_HANDLE_VALUE));
#else // !(_CRT_APP || _KERNELX)
    for(;;)
    {
...
HANDLE __CreateThread(LPSECURITY_ATTRIBUTES lpAttributes,
    ...
    LPDWORD threadId)
{
// TODO - KERNELX - Once _CRT_APP is not defined for kernelx, we should remove the second clause
#if defined(_CRT_APP) && !defined(_KERNELX)
    return __crtCreateWinRTThread(startAddress, param, threadId);
#else
...
// TODO - KERNELX - Once _CRT_APP is not defined for kernelx, we should remove the second clause
#if defined(_CRT_APP) && !defined(_KERNELX)
    __crtCloseWinRTThreadHandle(hThread);
#else
    CloseHandle(hThread);
#endif // _CRT_APP
}
...
// TODO - KERNELX - Once _CRT_APP is not defined for kernelx, we should remove the second clause
#if defined(_CRT_APP) && !defined(_KERNELX)
    return __crtWaitForWinRTThreadExit(hThread, timeout);
#else
