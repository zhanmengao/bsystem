...
// TODO - KERNELX - Once _CRT_APP is not defined for kernelx, we should remove the second clause
...
#endif // _CRT_APP
        {
#if defined(_CRT_APP)
            HANDLE newTimer; 
            if (!platform::__CreateTimerQueueTimer(&newTimer, 
                                    NULL,
                                    SchedulerBase::ThrottlerDispatchBridgeXP,
                                    this,
                                    dueTime,
            ...
            HANDLE oldTimer = InterlockedExchangePointer((PVOID volatile *)&m_hThrottlingTimer, newTimer);
            if (oldTimer != NULL)
            {
                platform::__DeleteTimerQueueTimer(NULL, oldTimer, NULL);
            }
#else // !_CRT_APP
