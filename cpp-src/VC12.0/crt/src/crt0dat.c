...
#if defined (_M_ARM)
/* These functions are used to set the default behaviour of memcpy operations on ARM.
 * If an external cache is available, memcpy() uses the *_integer functions.
 * Otherwise, memcpy() will keep the defaults, which are the *_neon functions.
 */
...
static void __cdecl _init_memcpy_functions(void);
...
*               when done (i.e., does NOT terminate the process).
*
*           _exit_app():  Performs all the C termination functions,
*               performs WinRT uninitialization,
*               and returns control to the caller when done.
*
*       Termination actions:
*
*           exit(), _cexit(), _exit_app():
*
...
#ifdef _CRT_APP
        /*
           The use of __fastfail here is a temporary workaround to ensure that exit() does not return.
           We cannot exit properly because there is no operating system API available to end execution.
           The __fastfail will raise a second-chance exception, causing the process to end execution, and invoking WER.         
        */
...
#ifdef _CRT_APP
        /*
           The use of __fastfail here is a temporary workaround to ensure that _exit() does not return.
           We cannot exit properly because there is no operating system API available to end execution.
           The __fastfail will raise a second-chance exception, causing the process to end execution, and invoking WER.         
        */
        __fastfail(FAST_FAIL_FATAL_APP_EXIT);
...
#endif  /* !defined(CRTDLL) && defined(_DEBUG) */
        }
...
#if (defined (_M_IX86) || defined (_M_X64)) && !defined (_CORESYS) && !defined (_CRT_APP)
        __crtLoadWinApiPointers();
#endif  /* (defined (_M_IX86) || defined (_M_X64)) && !defined (_CORESYS) && !defined (_CRT_APP) */
...
/***
*__copy_to_char() - Convert an wchar_t to ANSI/MBCS string.
*
*Entry:
*       const wchar_t *inString - input string
*       char **outString -  Pointer to converted char* string
*
*Exit:
*       outString points to newly allocated char output string
*       returns TRUE if successful
*       returns FALSE and sets errno if unsuccessful
*
*Exceptions:
...
BOOL __cdecl __copy_to_char(const wchar_t* inString, char** outString)
{
    ...
    /* validation section */
    _VALIDATE_RETURN_ERRCODE(inString != NULL, EINVAL);
    _VALIDATE_RETURN_ERRCODE(outString != NULL, EINVAL);
    ...
    *outString = NULL;
    ...
    /* get the buffer size needed for conversion */
    if  ( (len = WideCharToMultiByte(codePage, 0 /* Use default flags */, inString, -1, 0, 0, NULL, NULL) ) == 0 )
    {
        _dosmaperr(GetLastError());
        return FALSE;
    }

    /* allocate enough space for inString wide char */
    if ( (*outString = (char*)_malloc_crt( len * sizeof(char) ) ) == NULL )
    {
        /* malloc should set the errno */
        return FALSE;
    }
    ...
    /* now do the conversion */
    if ( WideCharToMultiByte(codePage, 0 /* Use default flags */, inString, -1, *outString, len, NULL, NULL) == 0 )
    {
        _dosmaperr(GetLastError());
        _free_crt(*outString);
        *outString = NULL;
...
/* Update memcpy function pointers */
static void __cdecl _init_memcpy_functions(
        void
        )
{
