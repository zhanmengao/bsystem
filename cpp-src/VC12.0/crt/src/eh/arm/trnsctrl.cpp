...
EHRegistrationNode *_GetEstablisherFrame(
    ...
    unsigned __int32 HandlerAdd, ImageBase;
    ...
    *pEstablisher = *pRN;
    for (index = num_of_try_blocks; index > 0; index--) {
        ...
        if (curState > TBME_HIGH(*pEntry) && curState <= TBME_CATCHHIGH(*pEntry)) {
            ...
            HandlerAdd = (*RtlLookupFunctionEntry(pDC->ControlPc, (PDWORD)&ImageBase, NULL)).BeginAddress;
...
__CRTDECL
_SetImageBase(
    unsigned __int32 ImageBaseToRestore
    )
{
...
__CRTDECL
_SetThrowImageBase(
    unsigned __int32 NewThrowImageBase
    )
{
...
__CxxFrameHandler3(
    ...
    _ThrowImageBase = (unsigned __int32)pExcept->params.pThrowImageBase;
                      ...
                      BOOL                  *pResult)
{
        ...
        _ThrowImageBase = (unsigned __int32)((EHExceptionRecord *)ExPtrs->ExceptionRecord)->params.pThrowImageBase;
...
extern "C" FRAMEINFO * _CreateFrameInfo(
...
//
extern "C" BOOL _IsExceptionObjectToBeDestroyed(
    PVOID pExceptionObject
) {
    FRAMEINFO * pFrameInfo;
...
//  case.
//
extern "C" void _FindAndUnlinkFrame(
    FRAMEINFO * pFrameInfo
) {
...
extern "C" void _UnwindNestedFrames(
    ...
    ExceptionRecord.ExceptionInformation[10] = -1;
...
void _CallMemberFunction0( void *pthis, void *pmfn )
{
        VOID (*OneArgFn)(PVOID Arg1);

        OneArgFn = (VOID(*)(PVOID)) pmfn;

        (*OneArgFn)(pthis);
}

void  _CallMemberFunction1( void *pthis, void *pmfn, void *pthat )
{
        VOID (*TwoArgFn)(PVOID Arg1, PVOID Arg2);

        TwoArgFn = (VOID(*)(PVOID, PVOID)) pmfn;

        (*TwoArgFn)(pthis, pthat);
}

void  _CallMemberFunction2( void *pthis, void *pmfn, void *pthat, int val2 )
{
        VOID (*ThreeArgFn)(PVOID Arg1, PVOID Arg2, int Val);

        ThreeArgFn = (VOID(*)(PVOID, PVOID, int)) pmfn;
