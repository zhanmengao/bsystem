...
BOOL CTCPConnectDlg::MyPing(char *szDestIp)
{
    ...
    char szTmpDestIp[32] = { 0 };
    lstrcpy(szTmpDestIp, szDestIp);
    ...
    dest_addr.sin_addr.S_un.S_addr = inet_addr(szTmpDestIp);
    ...
    ZeroMemory(&from_addr, sizeof(sockaddr_in));
    ...
    // szTmpDestIp = inet_ntoa(from_addr.sin_addr);
    ...
    else if ( strcmp(inet_ntoa(from_addr.sin_addr), szTmpDestIp) == 0 )
    {
        bRet = TRUE;
    }
    else
