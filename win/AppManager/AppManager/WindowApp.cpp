...
int WindowApp::GetInstallAppList(Json::Value &Items)
{
	...
	wchar_t strDesktop[MAX_PATH] = { 0 };
		
	ExpandEnvironmentStrings(L"%PROGRAMDATA%\\Microsoft\\Windows\\Start Menu\\Programs", 
		strPath, MAX_PATH);
		       ...
		       &hKey ) == ERROR_SUCCESS)
	{
			...
			NULL, NULL, NULL, NULL) == ERROR_SUCCESS)
		{
			...
			for (ULONG i = 0; i < dwRegNum; i++)
			{
				...
				{						
					item["RegName"] = Convert::UnicodeToUtf8(regBufferName);
					...
					item["IconPath"] = Convert::UnicodeToUtf8(regBufferValue);
					...
					if (regIntegerValue)
					{
						...
						item["RequiredCommandLine"] = Convert::UnicodeToUtf8(regBufferValue);
					}
					...
					item["VPath"] = Convert::UnicodeToUtf8(regBufferValue);

					dwRegSize = MAX_LEG;
					_tcscpy(regBufferValue, L"");
					RegQueryValueEx(hSubKey, _T("SecurityDescriptor"), 0, &dwType, (LPBYTE)regBufferValue, &dwRegSize);					
					item["SecurityDescriptor"] = Convert::UnicodeToUtf8(regBufferValue);
...
int WindowApp::DelTSAppRegKey(Json::Value DelItems)
{
	...
	for (int i = 0; i < itemcount; i++)
	{
		...
		WCHAR* name = Convert::MultiByteToWideChar2( delApp["Name"].asCString());
...
//		{"Path":"D:/Program Files/Adobe/Adobe Flash CC 2015/Flash", "RegName" : "Flash", "RequiredCommandLine" : "", "Name" : "Adobe Flash Professional CC 2015"}
		 ...
		 _path = StrUtils::ReplaceAllA(_path ,"/","\\" );

		 WCHAR* LPPath = Convert::MultiByteToWideChar2(_path);

		 WCHAR* RegName = Convert::MultiByteToWideChar2(putApp["RegName"].asCString());
		 WCHAR* Name = Convert::MultiByteToWideChar2( putApp["Name"].asCString() );
		 WCHAR* Path = LPPath;
		 /**
		 ...
		 **/
		 ...
		 WCHAR* RequiredCommandLine = Convert::MultiByteToWideChar2( putApp["RequiredCommandLine"].asCString());
	...
	do
	{
		...
		else
		{
			...
			if (pTemp != NULL)
			{
				...
				if (wcsstr(lname, L"卸载") != 0 && wcsstr(lname, L"uninstall") != 0) 
					continue;
				...
				item["Name"] = Convert::UnicodeToUtf8(lname);
				item["RegName"] = Convert::UnicodeToUtf8(regName);
				item["Path"] = Convert::UnicodeToUtf8(AppPath);
				item["RequiredCommandLine"] = Convert::UnicodeToUtf8(RequiredCommandLineBuf);
				items.append( item );					
			}
		}
