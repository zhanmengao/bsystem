...
class FifoCpp
{
    private IntPtr mWrite;
    private IntPtr mRead;
    public void InitServer(string readPipeName, string writePipeName)
    {
        bool bCreateRet = true;
        do
        {
            mRead = PipeNative.CreateNamedPipe(readPipeName,
                ...
                IntPtr.Zero);
            if (mRead.ToInt32() == PipeNative.INVALID_HANDLE_VALUE)
            {
                bCreateRet = false;
                break;
            }
            if (PipeNative.ConnectNamedPipe(mRead, IntPtr.Zero) == false)
            {
                bCreateRet = false;
                break;
            }
            ...
            {
                //打通管道
                if (PipeNative.WaitNamedPipe(writePipeName, 1000) == false)
                {
                    continue;
                }
                mWrite = PipeNative.CreateFile(writePipeName,
                    FileDesiredAccess.GENERIC_WRITE, FileShareMode.Zero, IntPtr.Zero,
                    FileCreationDisposition.OPEN_EXISTING, 0x00000080, 0);
                if (mWrite.ToInt32() == PipeNative.INVALID_HANDLE_VALUE)
                {
                    Thread.Sleep(1000);
                    continue;
                }
        ...
        if (bCreateRet)
        {
            Console.WriteLine("管道创建成功！");
        }
        else
        {
            Console.WriteLine("管道创建失败！");
        }
    }
    public void InitClient(string readPipeName, string writePipeName)
    {
        bool bCreateRet = true;
        do
        {
    ...
    uint wn;
    uint rn;
    public uint Write(string rData)
    {
        byte[] bData = ToByte(rData);
        if (PipeNative.WriteFile(mWrite, bData, (uint)bData.Length, out wn, IntPtr.Zero) == false)
        {
            return wn;
        }
        return 0;
    ...
    public string Read()
    {
        ...
        bool bRet = PipeNative.ReadFile(mRead, readBuffer, (uint)readBuffer.Length,
            out rn, IntPtr.Zero);
        if (bRet)
        {
            return ByteToString(readBuffer, rn);
        }
        else
        {
            return null;
    ...
    public void Close()
    {
        if (mRead.ToInt32() != PipeNative.INVALID_HANDLE_VALUE
            && mRead != IntPtr.Zero)
        {
            PipeNative.CloseHandle(mRead);
            mRead = IntPtr.Zero;
        }
        if (mWrite.ToInt32() != PipeNative.INVALID_HANDLE_VALUE
    && mWrite != IntPtr.Zero)
        {
            PipeNative.CloseHandle(mWrite);
            mWrite = IntPtr.Zero;
        }
    }
    private static string ByteToString(byte[] byteArr, uint size)
    {
        return System.Text.Encoding.GetEncoding("gb2312").GetString(byteArr, 0, (int)size);
    }
    private static byte[] ToByte(float num)
    {
    ...
    private static int ToInt(byte[] src, int offset)
    {
        int value;
