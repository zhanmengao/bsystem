...
bool CIOCPModel::_InitializeListenSocket()
{
	...
	//char * pFileName = T2A(m_strIP);
	//ServerAddress.sin_addr.s_addr = inet_addr(pFileName);
	//ServerAddress.sin_port = htons(m_nPort);

	sockaddr_in sin;
	sin.sin_family = AF_INET;
	sin.sin_port = htons(ZPORT);
	sin.sin_addr.S_un.S_addr = INADDR_ANY;

	// 绑定地址和端口
	if (SOCKET_ERROR == bind(m_pListenContext->m_Socket, (LPSOCKADDR)&sin, sizeof(sin)))
	{
...
bool CIOCPModel::_DoAccpet(PER_SOCKET_CONTEXT* pSocketContext, PER_IO_CONTEXT* pIoContext)
{
	...
	this->_ShowMessage(_T("客户端 %s:%d 信息：%s."), inet_ntoa(ClientAddr->sin_addr), ntohs(ClientAddr->sin_port), pIoContext->m_wsaBuf.buf);
