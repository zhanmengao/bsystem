...
BOOL ThreadPool::QueueTaskItem(TaskFun task, SOCKET param, TaskCallbackFun taskCb, BOOL longFun)
{
	waitTaskLock.Lock();
...
void ThreadPool::Thread::ExecuteTask(TaskFun task, SOCKET param, TaskCallbackFun taskCallback)
{
	busy = TRUE;
	this->task = task;
	this->param = param;
