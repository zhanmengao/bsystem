...
所以进程不断地有机会 "被" CPU光顾, 理论上可以做点其它事. 看上去 Linux非阻塞I/O 要比阻塞好, 但CPU会很大机率�1�3因socket没数据�1�3而空转. 虽然这个进程是爽了, 但是从整个机器的效率来说, 浪费更大了
...
可以用 "内存映射" 以及 DMA等方式来达到 "不用CPU去参与繁重的工作" 的目的. "内存映射�1�3" 是不用copy, 而DMA是有其它芯片来代替CPU处理.
