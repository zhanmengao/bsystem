...
BOOL CInstallDlg::OnInitDialog()
{
	...
	pPipe = new IPCAnon();
	HANDLE hRead, hWrite;
	pPipe->Init(hRead, hWrite);
	bool bRet = mProcess.InitProcess(_T(""), _T("powershell"), hRead, hWrite);
	pPipe->WriteData("Set-ExecutionPolicy UnRestricted");

	//mScriptList.SetExtendedStyle(mScriptList.GetStyle() | LVS_EX_CHECKBOXES);//前面有勾选格
	mScriptList.InsertColumn(0, _T("路径"), 0, 150);
	mScriptList.InsertColumn(2, _T("描述"), 0, 250);
	mScriptList.InsertColumn(1, _T("状态"), 0, 80);
	//读ini 加到列表
	ini.SetPath(PathHelper::GetModuleFolder() + _T("powershell.ini"));
	...
	while (rn <= 0)
	{
		rn = pPipe->ReadData(str);
	}
	SetOutputText(Convert::StringToWString(str).c_str());
...
void CInstallDlg::UpdateScriptState()
{
	...
	for (int i = 0; i < mScriptVec.size(); i++)
	{
		...
		mScriptList.SetItemText(i, 0, item.item.Script.c_str());
...
void CInstallDlg::ShellScripts()
{
	...
	while (mState == INSTALL_RUN)
	{
		...
		{
			SetOutputText((_T("开始安装	") + mCurrentInstall->mDesc + _T("	") +
				mCurrentInstall->item.Script + _T("	")).c_str());
			...
			mScriptList.SetItemText(index, 2, _T("正在安装"));
			//调用
			pPipe->WriteData(Convert::WStringToString(mCurrentInstall->item.Script).c_str());
			Sleep(10000);
			std::string str;
			int rn = pPipe->ReadData(str);
			SetOutputText((mCurrentInstall->item.Script + _T(" : ") + Convert::StringToWString(str)).c_str());

			SetOutputText((_T("完成安装	") + mCurrentInstall->mDesc + _T("	") +
				mCurrentInstall->item.Script).c_str());
