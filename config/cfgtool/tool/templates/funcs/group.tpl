...
// GetItem{{.FuncName}} {{.Desc}}; 获取{{.Key}}对应[]{{.ValueType}}; 传入调用方ctx参数, 如果不存在则传nil
func (c *{{.BuildClassName}}) {{.FuncName}}ItemList(ctx contextdefine.IContext, _{{.KeyLower}} {{.KeyType}}) (item []{{.ValueType}}, ok bool) {
	//复制
	m := c.{{.FuncName}}(ctx)
